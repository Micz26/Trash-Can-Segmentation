...
def base64_2_mask(s):
    ...
    mask = cv2.imdecode(n, cv2.IMREAD_UNCHANGED)[:, :, 3].astype(float)
    return mask


def submask_2_mask(submask, origin, height, width):
    mask = np.zeros((height + 2, width + 2), float)
    ox = int(origin[0] + 1)
    oy = int(origin[1] + 1)
    hy, wx = submask.shape
...
def create_mask_annotation(
    im_name, mask, image_id, category_id, annotation_id, is_crowd, show_vis
):
...
def create_image_entry(file_name, im_id, width, height):
    return {"file_name": file_name, "height": height, "width": width, "id": im_id}
...
def get_category_id_string(obj):
    ...
    if class_title == "rov":
        ...
        if not type_found:
            bio_fail.append(str(" : " + str(obj["tags"])))
            return None
            ...
            for tag in tags:
                if tag["name"] == "material":
                    if tag["value"] != "glass" and tag["value"] != "glass":
                        id_string = "trash_" + str(
                            tag["value"]
                        )  # Create cateogry_id key from trash_ + material value.
                    else:
                        id_string = "trash_etc"
...
def process_annotations(file_list, set_type):
    ...
    for ann_file in tqdm(file_list):
        ...
        with open("data/original_data/annotations/" + ann_file) as json_file:
            ...
            for obj in objects:
                ...
                if (
                    "instance" in obj.keys()
                ):  # If an object is bound to another, it has an instance tag.
                    ...
                    for o in objects:
                        if "instance" in o.keys() and o["instance"] == instance:
                            ...
                            add_submasks.append(
                                submask_2_mask(bmp, orig, height, width)
                            )
                            objects.remove(o)

                submask = base64_2_mask(
                    obj["bitmap"]["data"]
                )  # Get bitmap and origin for current object.
            ...
            copy(
                "data/original_data/images/" + ann_file[:-5],
                "data/"
                + dataset_version
                + "_version/"
                + set_type
        ...
        json.dump(
            {
                ...
                "categories": [
                    ...
                    {
                        "supercategory": "animal_starfish",
                        "id": 4,
                        "name": "animal_starfish",
                    },
                    {
                        "supercategory": "animal_shells",
                        "id": 5,
                        "name": "animal_shells",
                    },
...
if __name__ == "__main__":
    ...
    for filename in tqdm(os.listdir("data/original_data/annotations/")):
        if filename.endswith(".jpg.json"):
            total_imgs += 1
            images_list.append(filename)

            with open("data/original_data/annotations/" + filename) as json_file:
                data = json.load(json_file)
                objects = data["objects"]
                ...
                for obj in objects:
                    id_string = get_category_id_string(obj)
                    if id_string:
                        overall_objects[id_string] = (
                            overall_objects.get(id_string, 0) + 1
                        )
                        obj_dict[id_string] = obj_dict.get(id_string, 0) + 1

                        if id_string not in object_image_dict.keys():
                            object_image_dict[id_string] = list()

                        if filename not in object_image_dict[id_string]:
    ...
    for id_string in tqdm(
        sorted(overall_objects, key=lambda key: overall_objects[key])
    ):
                ...
                if image in images_list:
                    if current_objs[id_string] <= val_desired:
                        if swip_swop:
                            ...
                            for key in image_object_dict[
                                image
                            ].keys():  # Add object counts to current count.
    ...
    for key in sorted(overall_objects):
        percent = "     | {:.2f} -- {:.2f} |".format(
            float(train_obj_dict[key]) / float(overall_objects[key]),
            float(val_obj_dict[key]) / float(overall_objects[key]),
        )
        ...
        print(
            ...
            + key
            + "    | "
            + str(train_obj_dict[key])
            + "   | "
            + str(val_obj_dict[key])
            + "     | "
            + str(overall_objects[key])
            + percent
        )

    percent = "  Percentage: {:.2f}".format(
        (float(len(training_names)) / float(total_imgs))
    )
